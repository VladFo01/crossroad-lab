...
export class Pedestrian extends RoadUser {
  public move(): boolean | string {
    ...
    if (
      this.cell.getMatrix.getCell(xCurrent - 1, yCurrent) &&
      this.cell.getMatrix.getCell(xCurrent - 1, yCurrent).getCover.canWalk
    ) {
      Directions[0] = true;
      possibleDirAmount++;
    }
    if (
      this.cell.getMatrix.getCell(xCurrent, yCurrent - 1) &&
      this.cell.getMatrix.getCell(xCurrent, yCurrent - 1).getCover.canWalk
    ) {
      Directions[1] = true;
      possibleDirAmount++;
    }
